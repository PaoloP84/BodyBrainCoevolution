 ...
 */
bool Problem::readConfig(char* filename)
{
    char *s;
    ...
    if (fp != NULL)
    {
        ...
        while (fgets(buff, MAX_STR_LEN, fp) != NULL)
        {
            ...
            if (strcmp(name, "polelen2")==0)
                m_length_2 = strtod(value, &ptr);
            else if (strcmp(name, "polemass2")==0)
                m_masspole_2 = strtod(value, &ptr);
        }
        fclose (fp);
        return true ;
    }
...
void Problem::initEnvironment(char* filename)
{
    ...
    // read parameters from the configuration file
    bool ret = readConfig(filename);
    if (!ret)
        printf("ERROR: unable to read parameters from file %s!\n", filename);
    
    //printf("InitDpole() from file %s\n", filename);
    //printf("Double-pole: pole_length_2 %.2f pole_mass_2 %.2f\n", m_length_2, m_masspole_2);
}
...
void Problem::render()
{
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        else
        {
            dobjects[c+1] = 200.0 + (m_state[0] + 0.25) * 50.0;
            dobjects[c+2] = 150.0;
            dobjects[c+3] = 200.0 + (m_state[0] + 0.25 + (m_length_2 * 5.0 * cos(M_PI / 2.0 + m_state[4]))) * 50.0;
