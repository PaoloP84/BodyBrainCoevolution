...
class Embryo(WalkerBase):
  foot_list = ['torso', 'bbody1', 'bbody2', 'bbody3', 'fbody1', 'fbody2', 'fbody3']

  def __init__(self):
    WalkerBase.__init__(self, "embryo.xml", "torso", action_dim=6, obs_dim=27, power=0.9)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.1 else -1

class EmbryoHard(WalkerBase):
  foot_list = ['torso', 'bbody1', 'bbody2', 'bbody3', 'bbody4', 'bbody5', 'bbody6', 'fbody1', 'fbody2', 'fbody3', 'fbody4', 'fbody5', 'fbody6']

  def __init__(self):
    WalkerBase.__init__(self, "embryo.xml", "torso", action_dim=12, obs_dim=45, power=0.9)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.1 else -1

class EmbryoChallenging(WalkerBase):
  foot_list = ['torso', 'bbody1', 'bbody2', 'bbody3', 'bbody4', 'bbody5', 'bbody6', 'bbody7', 'bbody8', 'bbody9', 'bbody10', 'bbody11', 'bbody12', 'fbody1', 'fbody2', 'fbody3', 'fbody4', 'fbody5', 'fbody6', 'fbody7', 'fbody8', 'fbody9', 'fbody10', 'fbody11', 'fbody12']

  def __init__(self):
    WalkerBase.__init__(self, "embryo.xml", "torso", action_dim=24, obs_dim=81, power=0.9)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.1 else -1

class Sumo(WalkerBase):
  foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']

  def __init__(self):
    WalkerBase.__init__(self, "ant.xml", "torso", action_dim=8, obs_dim=31, power=2.5)
